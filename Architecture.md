...
### Ingestion
Kafka topics (`news_raw`, `semantic_groups`, `predictions`) with Avro schema registry, idempotent producers, transactional writes, schema contracts, and CI/CD schema validation.

### Processing
Ingestion validation → Canonicalization → Embedding service → Clustering → Feature engineering → Vector indexing → Validation and labeling pipeline.
...
### Lineage & Metadata
**OpenLineage + Marquez** with MLflow and Great Expectations integration.

### Verification Sources
ACLED, GDELT, CoinGecko with backup feeds, cached refresh, and freshness enforcement.

### Monitoring Stack
Prometheus, Grafana, Alertmanager, MLflow, EvidentlyAI, Great Expectations, Jaeger, OpenLineage, Loki, ELK.
...
## DATASET SCORING AND AUDIT LOGGING
...
## AUTOMATED RECOVERY AND FEED MONITORING
