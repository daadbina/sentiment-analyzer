#!/usr/bin/env python3
"""Check sentiment database for clustering schema."""

import asyncio
import asyncpg

async def main():
    # Connect to sentiment database (not sentiment_analyzer)
    conn = await asyncpg.connect(
        host='154.53.166.231',
        port=5432,
        user='adminsentiment',
        password='wp2400!!!!',
        database='sentiment'  # Note: sentiment, not sentiment_analyzer
    )
    ...
    try:
        ...
        """)
        print("Schemas in 'sentiment' database:")
        for row in schemas:
