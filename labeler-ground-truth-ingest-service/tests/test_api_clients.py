...
class TestGDELTFetcher:
    ...
    @pytest.mark.asyncio
    async def test_gdelt_parse_response(self):
        """Test GDELT response parsing."""
        import pandas as pd
        fetcher = GDELTFetcher()
        # GDELT returns a pandas DataFrame
        ...
        labels = await fetcher.parse_response(response)
        assert len(labels) == 1
        assert labels[0]["actor_a"] == "example.com"
        assert labels[0]["actor_b"] == "en"
    ...
    async def test_gdelt_parse_response_multiple(self):
        ...
        labels = await fetcher.parse_response(response)
        assert len(labels) == 2
        assert labels[0]["domain"] == "example.com"
        assert labels[1]["domain"] == "example2.com"
