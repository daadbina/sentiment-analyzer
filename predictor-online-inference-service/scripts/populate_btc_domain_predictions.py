"""
Populate BTC domain predictions with correct BTC features from parquet file.

This script:
1. Fetches all BTC domain predictions from PostgreSQL
...
async def populate_btc_domain_predictions():
    """Populate BTC domain predictions with correct BTC features."""
    
    logger.info("=" * 80)
    logger.info("Populating BTC Domain Predictions with BTC Features")
    logger.info("=" * 80)
    ...
    try:
        # Fetch all BTC domain predictions
        logger.info("\n" + "=" * 80)
        logger.info("Step 1: Fetch BTC domain predictions from PostgreSQL")
        logger.info("=" * 80)
        ...
        logger.info(f"\n📊 Found {len(predictions)} BTC domain predictions")
        ...
        for i, pred in enumerate(predictions, 1):
            ...
            # For BTC domain, features are stored directly (not nested like conflict domain)
            # Merge BTC features into current features
            updated_features = {**current_features, **btc_features_no_prefix}
        ...
        logger.info("SUMMARY")
        logger.info("=" * 80)
        logger.info(f"\n✅ Successfully populated BTC domain predictions with BTC features:")
...
if __name__ == "__main__":
    asyncio.run(populate_btc_domain_predictions())
