"""
Verify BTC domain predictions have correct BTC features.
"""
...
async def verify_btc_predictions():
    """Verify BTC domain predictions."""
    
    logger.info("=" * 80)
    logger.info("Verifying BTC Domain Predictions")
    logger.info("=" * 80)
    ...
    try:
        # Check PostgreSQL
        logger.info("\n" + "=" * 80)
        logger.info("PostgreSQL BTC Predictions")
        logger.info("=" * 80)
        ...
        for pred in predictions:
            ...
            # Check for BTC features (no nested structure for BTC domain)
            has_btc = 'btc_close' in features and features['btc_close'] != 0
        ...
        logger.info(f"\n📊 PostgreSQL Summary:")
        logger.info(f"   Total BTC predictions: {total}")
        ...
        # Check Neo4j
        logger.info("\n" + "=" * 80)
        logger.info("Neo4j BTC Predictions")
        logger.info("=" * 80)
        ...
        async with neo4j_driver.session(database=neo4j_database) as session:
            ...
            logger.info(f"\n📊 Neo4j Summary:")
            logger.info(f"   Total BTC prediction nodes: {total}")
            logger.info(f"   ✅ With BTC features: {with_btc} ({with_btc/total*100:.1f}%)")
            logger.info(f"   ❌ Without BTC features: {without_btc} ({without_btc/total*100:.1f}%)")
            ...
            if missing_nodes:
                ...
                for node in missing_nodes:
                    logger.info(f"   - ID: {node['id']}, Group: {node['group_id']}, Features: {node['features_count']}")
        ...
        if with_btc == total and without_btc == 0:
            logger.info("\n🎉 SUCCESS! All BTC predictions have correct BTC features!")
            logger.info("\n✅ PostgreSQL: Features stored directly WITHOUT prefix")
...
if __name__ == "__main__":
    asyncio.run(verify_btc_predictions())
