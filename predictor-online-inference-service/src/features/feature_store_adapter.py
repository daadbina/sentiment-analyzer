...
from ..exceptions import FeatureError, FeatureReconciliationError
...
class FeatureStoreAdapter:
    ...
    def __init__(self, config: FeastConfig, metrics: MetricsCollector):
        ...
        logger.info(
            "Initializing FeatureStoreAdapter",
            extra={"repo_path": config.repo_path}
        )
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            logger.error(
                "Failed to connect to Feast feature store",
                extra={"error": str(e)},
                exc_info=True
            )
            raise FeatureError(
    ...
    async def get_online_features(
        ...
        if not self._store:
            raise FeatureError(
                message="Feature store not connected",
                details={"operation": "get_online_features"}
            )
    ...
    async def get_offline_features(
        ...
        if not self._store:
            raise FeatureError(
                message="Feature store not connected",
                details={"operation": "get_offline_features"}
            )
    ...
    def get_feature_version(self, feature_name: str) -> Optional[str]:
        """
