...
class TestTrainerService:
    ...
    def mock_clients(self):
        """Create mock clients."""
        with patch('src.service.PostgreSQLClient') as mock_pg, \
             patch('src.service.FeastClient') as mock_feast, \
             patch('src.service.MLflowClient') as mock_mlflow, \
             patch('src.service.S3Client') as mock_s3, \
             patch('src.service.KafkaProducer') as mock_kafka:
            yield {
                'postgres': mock_pg,
                'feast': mock_feast,
                'mlflow': mock_mlflow,
    ...
    async def test_service_start(self, mock_clients):
        ...
        with patch.object(service, 'postgres_client', new_callable=AsyncMock), \
             patch.object(service, 'feast_client', new_callable=MagicMock), \
             patch.object(service, 'mlflow_client', new_callable=MagicMock):
    ...
    async def test_service_health_check(self, mock_clients):
        ...
        with patch.object(service, 'postgres_client', new_callable=AsyncMock) as mock_pg:
    ...
    async def test_service_train_pipeline(self, mock_clients):
        ...
        with patch.object(service, 'feature_retriever', new_callable=MagicMock) as mock_feat, \
             patch.object(service, 'label_retriever', new_callable=AsyncMock) as mock_label, \
             patch.object(service, 'preprocessor', new_callable=MagicMock) as mock_prep, \
             patch.object(service, 'splitter', new_callable=MagicMock) as mock_split, \
             patch.object(service, 'trainer', new_callable=MagicMock) as mock_train, \
             patch.object(service, 'evaluator', new_callable=MagicMock) as mock_eval:
    ...
    async def test_service_get_models(self, mock_clients):
        ...
        with patch.object(service, 'mlflow_client', new_callable=MagicMock) as mock_mlflow:
    ...
    async def test_service_get_drift_results(self, mock_clients):
        ...
        with patch.object(service, 'drift_detector', new_callable=MagicMock) as mock_drift:
    ...
    async def test_service_get_artifacts(self, mock_clients):
        ...
        with patch.object(service, 'artifact_manager', new_callable=MagicMock) as mock_art:
    ...
    async def test_service_promote_model(self, mock_clients):
        ...
        with patch.object(service, 'model_promoter', new_callable=MagicMock) as mock_prom:
    ...
    async def test_service_error_handling(self, mock_clients):
        ...
        with patch.object(service, 'feature_retriever', new_callable=MagicMock) as mock_feat:
            mock_feat.retrieve_features.side_effect = Exception("Feature retrieval failed")
